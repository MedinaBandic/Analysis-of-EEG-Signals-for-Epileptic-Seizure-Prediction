...
for i = 1:15
  ...
  if ((i~=6) && (i~=13))
   if i<10
        string = strcat('chb05_0', num2str(i), '_edfm', '.mat');
    else
        string = strcat('chb05_', num2str(i), '_edfm', '.mat');
    end
  end  
    x = load (string);
    sig = x.val(:,:);
...
%ictal_sig_1=load('chb05_16_edfm.mat');
%signal=[signal;ictal_sig_1.val(:,:)'];

ictal_sig_2=load('chb05_17_edfm.mat');
